  ...
  return (
    <aside className="w-64 bg-white border-r border-[#E0E0E0] flex flex-col h-full shrink-0">
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
        {NAV.map(item => {
          const active = pathname === item.href || (item.href !== '/dashboard' && pathname.startsWith(item.href) && !item.href.includes('admin'))
          return (
        ...
        <div className="pt-3 mt-3 border-t border-[#F5F5F5]">
          <p className="text-xs font-semibold text-[#B3B3B3] px-3 py-1 uppercase tracking-wider">Admin</p>
          {ADMIN_NAV.map(item => {
